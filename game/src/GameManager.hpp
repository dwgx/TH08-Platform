...
struct GameManager
{
    ...
    void AddToYoukaiGauge(i16 amount, i32 forceUpdate);

    ZunBool FinalBCleared(i32 team);
    ZunBool FinalBClearedWithAnyTeam();
    ZunBool FinalACleared(i32 team);
    ...
    void AddToClockTime(char clockTime);
    unsigned __int64 GetDeaths();
    u8 SetPower(i32 power);
    void AddScore(i32 score);
    u32 GetTimeOrbs();
    u32 GetLastSpellTimeOrbThreshold();
    i32 ScaleIntBasedOnRank(i32 min, i32 max);
    unsigned __int64 GetPower();
    f64 ScaleFloatBasedOnRank(f32 min, f32 max);
    i32 IsSoloHuman();
    i32 IsSoloYoukai();
    unsigned __int64 GetLives();
    void SetYoukaiGauge(u16 youkaiGauge);
    void AddToBombCount(i32 bombs);
    unsigned __int64 GetBombsRemaining();
    unsigned __int64 GetBombsUsed();
