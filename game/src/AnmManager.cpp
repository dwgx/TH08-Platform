...
ZunResult AnmManager::CreateTextureFromFile(IDirect3DTexture8 **outTexture, i32 format, i32 colorKey)
{
    ...
    entry = (AnmEntry *)outTexture;
    format = GetAnmFormat(format);
    if (D3DXCreateTextureFromFileInMemoryEx(g_Supervisor.d3dDevice, entry->rawData, entry->size, D3DX_DEFAULT,
                                            D3DX_DEFAULT, D3DX_DEFAULT, 0, g_TextureFormatD3D8Mapping[format],
                                            D3DPOOL_MANAGED, D3DX_FILTER_NONE, D3DX_FILTER_NONE, colorKey, NULL, NULL,
                                            outTexture) != D3D_OK)
    {
        return ZUN_ERROR;
    }
                                        ...
                                        AnmRawEntry *rawEntry)
{
    ...
    *sprites += rawEntry->numSprites;
    *scripts += rawEntry->numScripts;

    anmLoaded->textures[entryNumber].rawData = NULL;
    anmLoaded->textures[entryNumber].size = 0;
    ...
    path = (const char *)(((u8 *)rawEntry) + rawEntry->nameOffset);
    if (path[0] == '@')
    {
        return ZUN_SUCCESS;
    }

    anmLoaded->textures[entryNumber].rawData = FileSystem::OpenFile(path, &anmLoaded->textures[entryNumber].size, 0);
    if (anmLoaded->textures[entryNumber].rawData == NULL)
    {
...
void AnmManager::FUN_00466310(IDirect3DTexture8 *outTexture, i32 x, i32 y, i32 width, i32 height, i32 fontWidth,
...
void AnmManager::FUN_00466d70(i32 arg1, i32 arg2, i32 arg3, i32 arg4, i32 arg5, i32 arg6, i32 arg7)
{
}
